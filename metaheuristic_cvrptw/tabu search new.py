...
def generate_neighbors(solution, vehicles, nodes, tabu_list, max_capacity_w, demands_w, dist_matrix):
    """
    Generate neighbors for the given solution using a sequential approach.
    Applies relocation, swap, 2-opt, and merging/splitting of routes incrementally.
    """
    s_t = time.time()
    neighbors = []
    visited_solutions = set()  # To avoid duplicate neighbors

    # Cache current weights and costs for each vehicle
    route_weights = {
        v: sum(demands_w[node] for node in solution[v] if node != 0) for v in vehicles
    }

    def is_valid_move(new_solution):
        """
        Check if the move creates a valid and non-duplicate solution.
        """
        visited_nodes = {node for route in new_solution.values() for node in route if node != 0}
        return (
            len(visited_nodes) == len(nodes)
            and new_solution not in tabu_list
            and frozenset(tuple(new_solution[v]) for v in vehicles) not in visited_solutions
        )

    def add_neighbor(new_solution):
        """
        Add a valid neighbor to the neighbors list and mark it as visited.
        """
        if is_valid_move(new_solution):
            neighbors.append(new_solution)
            visited_solutions.add(frozenset(tuple(new_solution[v]) for v in vehicles))

    # Sequentially apply each move type
    for move_type in ["relocation", "swap", "2-opt", "merge-split"]:
