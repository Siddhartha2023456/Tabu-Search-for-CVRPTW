...
def generate_neighbors(solution, vehicles, nodes, tabu_list, max_capacity_w, demands_w, dist_matrix, time_matrix, start_time, finish_time, service_time):
    ...
    # Swap: Swap two customers between different vehicles
    for v1, v2 in itertools.permutations(vehicles, 2):
...
def diversify_solution(solution, nodes, percentage=0.4):
    """
    Diversifies the solution by removing 'percentage' of customers from routes and reinserting them randomly.
    """
    num_to_remove = int(len(nodes) * percentage)
    removed_nodes = []
    
    # Collect nodes to remove
    for v in solution:
        if len(solution[v]) > 2:  # Ignore empty or only-depot routes
            for _ in range(num_to_remove // len(solution)):
                if len(solution[v]) > 2:  # Ensure we don't remove depot
                    removed_node = random.choice(solution[v][1:-1])  # Avoid depot
                    removed_nodes.append(removed_node)
                    solution[v].remove(removed_node)
    
    # Reinsert nodes randomly
    for node in removed_nodes:
        v = random.choice(list(solution.keys()))
        if len(solution[v]) > 1:
            insert_pos = random.randint(1, len(solution[v]) - 1)
            solution[v].insert(insert_pos, node)
    
    return solution

def tabu_search_with_diversification(
    ...
    """
    Tabu Search with Diversification Strategy
    """
    ...
    for iteration in range(max_iter):
        ...
        if no_improvement_count >= 3 and not diversification_applied:
            print("Applying diversification...")
            current_solution = diversify_solution(current_solution, nodes)
            no_improvement_count = 0
            diversification_applied = True  # Ensure diversification is applied only once
        ...
        for neighbor in neighbors:
            feasible = all(
                is_valid_route(neighbor[v], demands_w, max_capacity_w[v], time_matrix, start_time, finish_time, service_time)
                for v in vehicles
            )
...
# # Run the updated tabu search
# tabu_search_with_diversification(
#     nodes, vehicles, dist_matrix, demands_w, max_capacity_w, Q1, var_cost, fixed_cost,
#     max_iter=50, tabu_tenure=10, time_matrix=time_matrix, start_time=start_time, finish_time=finish_time,
#     service_time=service_time, time_limit=50
# )

# Start measuring total runtime
start_time_total = time.time()

best_solution, best_cost= tabu_search_with_diversification(nodes, vehicles, dist_matrix, demands_w, max_capacity_w, Q1=Q1, var_cost=var_cost, fixed_cost=fixed_cost,
    max_iter=50, tabu_tenure=10, time_matrix=time_matrix, start_time=start_time, finish_time=finish_time,service_time=service_time,time_limit=3000)
